...
def test_technology_assumptions_use_techcat_schema_and_gas_price_formula():
    ...
    wind = pd.DataFrame(
        [[2030, None, 30, 1.0, 2.0, 10_000], [2040, None, 30, 0.9, 2.0, 9_000]],
        columns=SCHEMA_COLUMNS,
    )
    solar = pd.DataFrame(
        [[2030, None, 40, 0.5, None, 5_000], [2040, None, 40, 0.4, None, 4_000]],
        columns=SCHEMA_COLUMNS,
    )
    gas = pd.DataFrame(
        [[2030, 0.4, 25, 0.6, 4.0, 8_000], [2040, 0.5, 25, 0.5, 3.0, 7_000]],
        columns=SCHEMA_COLUMNS,
    )
    ...
    assert gas_assumption.marginal_cost_by_period[2030] == 80 / 0.4 + 4
    assert gas_assumption.marginal_cost_by_period[2050] == 85 / 0.5 + 3
